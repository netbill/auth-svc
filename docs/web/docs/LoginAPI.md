...
[**AuthSvcV1LoginPost**](LoginAPI.md#AuthSvcV1LoginPost) | **Post** /auth-svc/v1/login/ | Login by email
...
> TokensPair AuthSvcV1LoginPost(ctx).LoginByEmail(loginByEmail).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.LoginAPI.AuthSvcV1LoginPost(context.Background()).LoginByEmail(loginByEmail).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `LoginAPI.AuthSvcV1LoginPost``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `AuthSvcV1LoginPost`: TokensPair
	fmt.Fprintf(os.Stdout, "Response from `LoginAPI.AuthSvcV1LoginPost`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiAuthSvcV1LoginPostRequest struct via the builder pattern
